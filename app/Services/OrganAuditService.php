...
final class OrganAuditService
{
    public function __construct(private OrganAuditRepository $audits)
    {
    }
     ...
     * }
     */
    public function profileData(int $organId, array $inputFilters, int $page = 1, int $perPage = 12): array
    {
        $filters = $this->normalizeFilters($inputFilters);
        $page = max(1, $page);
        $perPage = max(5, min(50, $perPage));

        $result = $this->audits->paginateByOrgan($organId, $filters, $page, $perPage);
        ...
        return [
            ...
            'filters' => $filters,
            'options' => [
                'entities' => $this->audits->entitiesByOrgan($organId),
                'actions' => $this->audits->actionsByOrgan($organId, $filters['entity'] !== '' ? $filters['entity'] : null),
            ],
        ];
    }
     ...
     */
    public function exportRows(int $organId, array $inputFilters, int $limit = 2000): array
    {
        $filters = $this->normalizeFilters($inputFilters);
        $limit = max(1, min(5000, $limit));

        return [
            'rows' => $this->audits->listByOrgan($organId, $filters, $limit),
            'filters' => $filters,
        ];
    }
