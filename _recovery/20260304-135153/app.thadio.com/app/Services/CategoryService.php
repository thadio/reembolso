...
{
    /**
     * Validate category data for catalog_categories schema.
     * 
    ...
    public function validate(array $input): array
    {
        ...
        // Parse parent_id (nullable)
        $parentId = null;
        if (isset($input['parent_id']) && $input['parent_id'] !== '') {
            $parentId = (int) $input['parent_id'];
            // Prevent self-referencing
            if (isset($input['id']) && (int) $input['id'] === $parentId) {
                $errors[] = 'Categoria não pode ser pai de si mesma.';
                $parentId = null;
            }
        }

        // Parse position (optional, for ordering)
        $position = null;
