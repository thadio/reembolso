...
<?php
  ...
  $formTitle = $editing ? 'Editar Categoria' : 'Nova Categoria';
  $submitLabel = $editing ? 'Atualizar Categoria' : 'Criar Categoria';
  $currentId = (int)($formData['id'] ?? 0);
?>
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:24px;">
  ...
  <a class="btn ghost" href="colecao-list.php">Voltar para lista</a>
</div>
...
<form action="colecao-cadastro.php<?php echo $editing && $currentId > 0 ? '?id=' . $currentId : ''; ?>" method="post" class="form-grid">
  <?php if ($editing && $currentId > 0): ?>
    ...
    <div class="form-row">
        ...
        <select id="parent_id" name="parent_id">
          ...
          <?php foreach ($categories as $category): ?>
            <?php 
              ...
              if ($editing && $catId === $currentId) continue;
            ?>
            <option 
              value="<?php echo $catId; ?>"
              <?php echo isset($formData['parent_id']) && (int)$formData['parent_id'] === $catId ? 'selected' : ''; ?>
            >
              <?php echo $esc($category['name']); ?>
            </option>
          <?php endforeach; ?>
        </select>
        <small class="form-help">Categorias podem ter uma categoria pai para criar hierarquia</small>
      </div>
      ...
      <div class="form-group">
        ...
        <input 
          ...
          id="position" 
          name="position" 
          value="<?php echo $esc($formData['position'] ?? '0'); ?>"
          min="0"
          step="1"
        >
        <small class="form-help">Ordem de exibição (menor número aparece primeiro)</small>
      </div>
    </div>
  </fieldset>
  ...
  <div class="form-actions" style="display:flex;gap:12px;justify-content:flex-end;margin-top:24px;">
    <a href="colecao-list.php" class="btn ghost">Cancelar</a>
