...
<?php
  ...
  $rangeStart = $totalCategories > 0 ? (($page - 1) * $perPage + 1) : 0;
  $rangeEnd = $totalCategories > 0 ? min($totalCategories, $page * $perPage) : 0;
  $buildLink = function (int $targetPage) use ($perPage, $filters, $sortKey, $sortDir, $columnFilterParams): string {
    ...
    return 'colecao-list.php?' . http_build_query($query);
  };
?>
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
  <div>
    <h1>Categorias</h1>
    <div class="subtitle">Catálogo interno de categorias</div>
  </div>
  <?php if ($canCreate): ?>
...
<div class="table-tools">
  <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
    <input type="search" data-filter-global placeholder="Buscar em todas as colunas" aria-label="Busca geral em categorias" value="<?php echo $esc($filters['search'] ?? ''); ?>">
  ...
  <form method="get" id="perPageForm" style="display:flex;gap:8px;align-items:center;">
    ...
    <?php endif; ?>
    <?php if (!empty($filters['parent_id'])): ?>
      <input type="hidden" name="parent_id" value="<?php echo (int) $filters['parent_id']; ?>">
    <?php endif; ?>
    ...
    <span style="font-size:13px;color:var(--muted);">Mostrando <?php echo $rangeStart; ?>–<?php echo $rangeEnd; ?> de <?php echo $totalCategories; ?></span>
  </form>
</div>
  ...
  <table data-table="interactive" data-filter-mode="server">
    <thead>
      <tr>
        ...
        <th data-sort-key="parent_name" aria-sort="none">Categoria Pai</th>
      ...
      <tr class="filters-row">
        ...
        <th><input type="search" data-filter-col="parent_name" data-query-param="filter_parent_name" value="<?php echo $esc($columnFilterParams['filter_parent_name'] ?? ''); ?>" placeholder="Filtrar categoria pai" aria-label="Filtrar categoria pai"></th>
    ...
    <tbody>
      <?php if (empty($rows)): ?>
        <tr class="no-results"><td colspan="7">Nenhuma categoria encontrada.</td></tr>
      <?php else: ?>
        <?php foreach ($rows as $row): ?>
          <?php 
            $rowLink = $canEdit ? 'colecao-cadastro.php?id=' . (int) $row['id'] : '';
          ...
          <tr<?php echo $rowLink !== '' ? ' data-row-href="' . $esc($rowLink) . '"' : ''; ?>>
            ...
            <td data-value="<?php echo $esc($row['parent_name'] ?? ''); ?>"><?php echo $esc($row['parent_name'] ?? '—'); ?></td>
            ...
            <td class="col-actions">
              <?php if ($canDelete): ?>
                <div class="actions">
                  <form method="post" onsubmit="return confirm('Arquivar esta categoria?');" style="margin:0;">
