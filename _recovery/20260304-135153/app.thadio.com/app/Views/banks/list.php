...
?>
<?php
  $canCreate = userCan('banks.create');
  $canEdit = userCan('banks.edit');
  $canDelete = userCan('banks.delete');
?>
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
  <div>
    <h1>Bancos</h1>
    <div class="subtitle">Cadastre os bancos usados para contas e PIX.</div>
  </div>
  <?php if ($canCreate): ?>
    <a class="btn primary" href="banco-cadastro.php">Novo banco</a>
  <?php endif; ?>
</div>
...
<div class="table-tools">
  <input type="search" data-filter-global placeholder="Buscar em todas as colunas" aria-label="Busca geral em bancos">
  ...
  <table data-table="interactive">
    <thead>
      <tr>
        ...
        <th data-sort-key="code" aria-sort="none">Código</th>
      ...
      <tr class="filters-row">
        ...
        <th><input type="search" data-filter-col="code" placeholder="Filtrar código" aria-label="Filtrar código"></th>
    ...
    <tbody>
      <?php if (empty($rows)): ?>
        <tr class="no-results"><td colspan="6">Nenhum banco cadastrado.</td></tr>
      <?php else: ?>
        <?php foreach ($rows as $row): ?>
          <?php $rowLink = $canEdit ? 'banco-cadastro.php?id=' . (int) $row['id'] : ''; ?>
          <tr<?php echo $rowLink !== '' ? ' data-row-href="' . $esc($rowLink) . '"' : ''; ?>>
            ...
            <td class="col-actions">
              <?php if ($canDelete): ?>
                <div class="actions">
                  <?php if ($canDelete): ?>
                    <form method="post" onsubmit="return confirm('Excluir este banco?');" style="margin:0;">
