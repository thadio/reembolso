...
<div>
  <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
    <div>
      <h1>Cadastro de Padrao de Identificacao</h1>
      <div class="subtitle">Defina padroes usados no campo de identificacao dos cupons/creditos.</div>
    </div>
    <span class="pill">
      <?php echo $editing ? 'Editando #' . $esc((string) $formData['id']) : 'Novo padrao'; ?>
    </span>
  </div>
  ...
  <form method="post" action="cupom-credito-identificacao-cadastro.php">
    <input type="hidden" name="id" value="<?php echo $esc((string) $formData['id']); ?>">
    <div class="grid">
      <div class="field">
        <label for="label">Identificacao *</label>
        <input type="text" id="label" name="label" required maxlength="160" value="<?php echo $esc($formData['label']); ?>" placeholder="Ex: Cupom promocional">
      </div>
      <div class="field">
    ...
    <div class="footer">
      ...
      <button class="primary" type="submit">Salvar padrao</button>
    </div>
  </form>
</div>
