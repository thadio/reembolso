...
?>
<?php
  $canCreate = userCan('voucher_identification_patterns.create');
  $canEdit = userCan('voucher_identification_patterns.edit');
  $canDelete = userCan('voucher_identification_patterns.delete');
?>
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
  <div>
    <h1>Padroes de identificacao</h1>
    <div class="subtitle">Padronize a identificacao usada no cadastro de cupom/credito.</div>
  </div>
  <?php if ($canCreate): ?>
    <a class="btn primary" href="cupom-credito-identificacao-cadastro.php">Novo padrao</a>
  <?php endif; ?>
</div>
...
<div class="table-tools">
  <input type="search" data-filter-global placeholder="Buscar em todas as colunas" aria-label="Busca geral em padroes">
  ...
  <table data-table="interactive">
    <thead>
      <tr>
        ...
        <th data-sort-key="label" aria-sort="none">Identificacao</th>
      ...
      <tr class="filters-row">
        ...
        <th><input type="search" data-filter-col="label" placeholder="Filtrar identificacao" aria-label="Filtrar identificacao"></th>
    ...
    <tbody>
      <?php if (empty($rows)): ?>
        <tr class="no-results"><td colspan="5">Nenhum padrao cadastrado.</td></tr>
      <?php else: ?>
        <?php foreach ($rows as $row): ?>
          <?php $rowLink = $canEdit ? 'cupom-credito-identificacao-cadastro.php?id=' . (int) $row['id'] : ''; ?>
          <tr<?php echo $rowLink !== '' ? ' data-row-href="' . $esc($rowLink) . '"' : ''; ?>>
            ...
            <td class="col-actions">
              <?php if ($canDelete): ?>
                <div class="actions">
                  <?php if ($canDelete): ?>
                    <form method="post" onsubmit="return confirm('Excluir este padrao?');" style="margin:0;">
